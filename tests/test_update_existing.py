...
def test_update_existing_merges_into_new_filename(monkeypatch, tmp_path):
    ...
    downloads = tmp_path / "downloads" / "bingx" / "swap"

    old_name = "BTC-USDT_1m_2020-01-01_2020-01-01_bingx_swap.csv"
    ...
    def fake_fetch_to_file_with_resume(source, market, symbol, interval, fromdate, todate, target_path):
        assert source == "bingx"
        assert market == "swap"
    ...
    bingx_fetch.update_existing_files(market="swap", source="bingx")

    new_path = downloads / "BTC-USDT_1m_2020-01-01_2020-01-02_bingx_swap.csv"
    ...
    assert not (downloads / "BTC-USDT_1m_2020-01-01_update_bingx_swap.part.csv").exists()
...
def test_update_existing_keeps_part_on_interruption(monkeypatch, tmp_path):
    ...
    old_name = "ETH-USDT_1m_2020-01-01_2020-01-01_bingx_swap.csv"
    old_path = downloads / old_name
    ...
    assert not (downloads / "ETH-USDT_1m_2020-01-01_2020-01-02_bingx_swap.csv").exists()
    assert (downloads / "ETH-USDT_1m_2020-01-01_update_bingx_swap.part.csv").exists()
...
def test_fetch_to_file_with_resume_continues_from_existing_part(monkeypatch, tmp_path):
    ...
    part_path = downloads / "XRP-USDT_1m_2020-01-01_update_bingx_swap.part.csv"
    _write_csv(part_path, [[1, 1, 1, 1, 10, "2020-01-01 00:00:00"]])

    calls = []

    def fake_fetch_klines(source, market, symbol, interval, start_ms, end_ms=None, limit=None):
        calls.append((source, start_ms))
        return [
            {
                "open": 2,
                "close": 2,
    ...
    assert calls[0] == ("bingx", expected_start)
...
def test_update_existing_uses_multithreading_by_default(monkeypatch, tmp_path):
    _prepare_downloads(monkeypatch, tmp_path)
    downloads = tmp_path / "downloads" / "bingx" / "swap"
    (downloads / "BTC-USDT_1m_2020-01-01_2020-01-01_bingx_swap.csv").write_text(
        "time\n2020-01-01 00:00:00\n", encoding="utf-8"
    )
    (downloads / "ETH-USDT_1m_2020-01-01_2020-01-01_bingx_swap.csv").write_text(
        "time\n2020-01-01 00:00:00\n", encoding="utf-8"
    )
    ...
    assert sorted(processed) == sorted(
        [
            "BTC-USDT_1m_2020-01-01_2020-01-01_bingx_swap.csv",
            "ETH-USDT_1m_2020-01-01_2020-01-01_bingx_swap.csv",
        ]
    )


def test_update_existing_runs_all_sources_and_markets_when_not_specified(monkeypatch, tmp_path):
    _prepare_downloads(monkeypatch, tmp_path)
    called = []
    ...
    assert called == [
        str(tmp_path / "downloads" / "bingx" / "spot"),
        str(tmp_path / "downloads" / "bingx" / "swap"),
        str(tmp_path / "downloads" / "binance" / "spot"),
        str(tmp_path / "downloads" / "binance" / "swap"),
    ]
...
def test_update_existing_can_run_sequentially(monkeypatch, tmp_path):
    ...
    bingx_fetch.update_existing_files(multithreading=False, market="swap", source="bingx")
